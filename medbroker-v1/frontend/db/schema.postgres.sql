...
CREATE TABLE IF NOT EXISTS Task (
    ...
    -- Nullable (v2.8, 30 Jul 2026, §69/migration 014): who created this
    -- task — always NULL for the five system-generated trigger rules
    -- (no human creator), always populated for a manually created one.
    -- Separate concern from assignedToId (who it's FOR); a Supervisor
    -- creating a task for someone outside their own reporting line still
    -- needs to see it themselves, which is what this column enables.
    createdById     UUID            NULL,
    -- Nullable (v2.7, 28 Jul 2026, §56/migration 013): always populated for
    -- the five system-generated trigger rules (taskService.js), always NULL
    -- for a manually created task (POST /api/tasks) — NewTaskModal never
    -- links a Lead/Appointment, it only has title/detail/category/etc.
    entityType      VARCHAR(50)     NULL,
    entityId        UUID            NULL,
    ...
    -- §138 (12 Aug 2026, migration 024) — set when a Callback-type task is
    -- auto-completed by a new call attempt against the same lead. See that
    -- migration's own header for the full reasoning (link, not a copied
    -- snapshot). NULL for every other task.
    resolvedByCallAttemptId UUID    NULL,
    createdAt       TIMESTAMPTZ     NOT NULL DEFAULT NOW(),
    updatedAt       TIMESTAMPTZ     NOT NULL DEFAULT NOW(),
    ...
    CONSTRAINT FK_Task_CreatedBy  FOREIGN KEY (createdById) REFERENCES "User"(id),
    CONSTRAINT FK_Task_ResolvedByCallAttempt FOREIGN KEY (resolvedByCallAttemptId) REFERENCES CallAttempt(id) ON DELETE SET NULL,
    CONSTRAINT CK_Task_EntityType CHECK (entityType IN ('Lead', 'Appointment', 'SubjectAccessRequest')),
    -- 'Manual' added (§56) — a manually created task isn't generated by any
    -- of the five system trigger rules, so it needs its own type value.
    -- 'Sar' added (§12b, 21 Aug 2026, migration 037) — the sixth
    -- system-generated type, for the SAR-linked redirect-only task
    -- createSarRequest() now creates alongside every request.
    CONSTRAINT CK_Task_Type       CHECK (type IN ('Callback', 'Appointment', 'Reschedule', 'Reminder', 'Outcome', 'Manual', 'Sar')),
    CONSTRAINT CK_Task_Priority   CHECK (priority IN ('High', 'Medium', 'Low'))
);
...
-- POPIA Subject Access Request tracking (v3.0, 1 Aug 2026, §79/migration
-- 017). Always tied to a Lead — MedBroker's primary holder of personal
-- information about a data subject; if the requester isn't in the
-- system as a Lead, there's nothing to compile.
