/**
 * handlers/integrationHandlers.js — NEW, §134 (6 Aug 2026). EXTENDED
 * §135 (7 Aug 2026) for Paystack.
 * Backs the Integrations settings page (App Admin → Integrations) —
 * Stripe + Paystack + SMTP credentials. Routed through system-config.js
 * as a slug sub-tree (GET /api/system-config/integrations, PUT
 ...
 * payment provider secret key or SMTP password is not the same category
 * of "config" as a call-attempt limit; nothing in this file is readable
 * by anyone below GlobalAdmin, GET included.
 */
...
import { UpdateStripeCredentialsSchema, UpdateSmtpCredentialsSchema, UpdatePaystackCredentialsSchema } from '../models/integration.js';

const SCHEMAS = {
  stripe:   UpdateStripeCredentialsSchema,
  smtp:     UpdateSmtpCredentialsSchema,
  paystack: UpdatePaystackCredentialsSchema,
};

/** GET /api/system-config/integrations — masked status for all providers. */
export async function handleIntegrationsStatus(req, res) {
  ...
  try {
    ...
    const [stripe, smtp, paystack] = await Promise.all([
      getMaskedStatus('stripe'), getMaskedStatus('smtp'), getMaskedStatus('paystack'),
    ]);
    return res.status(200).json({ stripe, smtp, paystack });
 ...
 * provider ('stripe' | 'smtp' | 'paystack'). Audit log records WHICH
 * FIELDS were changed, never the values — same principle as never
 * writing special PI to logs elsewhere in this app, just applied to
 * secrets instead of POPIA data.
 */
